...
static sgx_status_t SGX_CDECL sgx_hello_world(void* pms)
{
...
static sgx_status_t SGX_CDECL sgx_match_regex(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	match_regex();
...
static sgx_status_t SGX_CDECL sgx_enclave_ra_init(void* pms)
{
	...
	ms->ms_retval = enclave_ra_init(ms->ms_key, _in_ctx);
	if (_in_ctx) {
...
SGX_EXTERNC const struct {
	...
	7,
	{
		{(void*)(uintptr_t)sgx_hello_world, 0, 0},
		{(void*)(uintptr_t)sgx_match_regex, 0, 0},
